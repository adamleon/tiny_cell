...
**Where we are** (update this line when a milestone lands): Steps 1–5 done. Step-6 Phase 1+2 on `dev`. Step-6 **M1 (port regions) and M2 (belt geometry) COMPLETE on `dev`** (17/17 green). M1: annulus transport endpoints are Layer-3 placer variables, polar `(r, θ)` with `r` hard-bounded to the reach band. M2: transports are routed as real placed belts **post-solve** — `BeltSpec` catalog (`core/model/belt.hpp` + `io::load_belt_catalog`), `route_belts()` (`solver/belt_routing`) builds a `PlacedBelt` per edge picking the cheapest catalog belt covering the routed distance, with belt-vs-station collision flagged (SAT); `solve_workflow` draws them. **A `BeltStrategy` OR-tree competitor was deliberately NOT built** — belt length is only known post-placement and magic transport is free, so belt geometry is a post-solve routing pass (see `decisions.md` "Belt geometry: PlacedBelt sibling + post-solve sequential routing"). **The next code is M3 — realistic palletizer scenario** (KUKA arm + belt catalog, domain-credible clearances, `validate_workflow()`; `roadmap.md`). **See `HANDOFF.md` for the M3 starting plan** (where things live, known issues to resolve, build loop).
