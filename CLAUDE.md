...
**Where we are** (update this line when a milestone lands): Steps 1–5 done. Step-6 Phase 1+2 on `dev`. Step-6 **M1 (port regions) in progress**: M1.1–M1.3 committed and green. **M1.4 (annulus port becomes a placer variable) is implemented and green in the working tree, NOT yet committed** — annulus endpoints are optimised as polar `(r, θ)` with `r` hard-bounded to the reach band (an adversarial review showed the originally-planned soft penalty let the port reach ~2× the arm's reach at default weights; see `decisions.md` "Annulus reach-band enforced as a HARD polar (r, θ) box"). **The next code is M1.5 — demo + SVG wiring** (draw the solved `item_in` landing inside its reach ring; copy `reach_min`/`reach_max` into the demo's `TransportConstraint`). See `HANDOFF.md` for the live plan. (`roadmap.md`.)
