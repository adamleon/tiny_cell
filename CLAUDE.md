...
**Where we are** (update this line when a milestone lands): **MVP COMPLETE — steps 1–6 all done on `dev`** (catalog/strategies → brute-force enumerator → item-state → Layer-2 allocation → Layer-3 NLP placement → step-6 **M1** reach-annulus ports, **M2** real placed belts, **M3** realistic palletizer scenario [the MVP go/no-go — **PASSED**], **M4** multi-equipment cells + convex-polygon narrow-phase overlap + dual-pallet intra-station `StationTemplate`). **19/19 `ctest` green; `dev` == `origin/dev`.** **The project is now at a PHASE BOUNDARY — the next leg is POST-MVP. Read `HANDOFF.md` first**: it carries the candidate next legs (interactive `render`/`gui`, the `sync`/`ecs` registry boundary, the parked LNS outer loop on `feature/step-6-lns`, the deferred M4 increments), the M1–M4 detail, where things live, and the build loop. The anti-stray discipline below still governs HOW to build; the MVP-phase prohibition on `sync`/`ecs`/`render`/`gui` (below + §6) **lifts for whichever single post-MVP leg you open** — build that one crudest-concrete-first, never all four speculatively.
