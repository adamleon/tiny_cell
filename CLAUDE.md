...
**Where we are** (update this line when a milestone lands): Steps 1–5 done. Step-6 Phase 1+2 on `dev`. Step-6 **M1 (port regions) COMPLETE on `dev`**: M1.1–M1.5 committed and green (16/16). Annulus transport endpoints are Layer-3 placer variables, optimised as polar `(r, θ)` with `r` hard-bounded to the reach band; the `solve_workflow` demo wires the band through and draws `item_in` landing at its reach edge toward the feeder. (An adversarial review corrected the originally-planned soft-penalty enforcement, which let the port reach ~2× the arm's reach — see `decisions.md` "Annulus reach-band enforced as a HARD polar (r, θ) box".) **The next code is M2 — belt geometry** (BeltSpec + BeltStrategy, `PlacedBelt`, sequential routing; `roadmap.md`).
