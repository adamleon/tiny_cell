...
**Where we are** (update this line when a milestone lands): Steps 1–5 done. Step-6 M1+M2 on `dev`. Step-6 **M3 (realistic palletizer scenario) COMPLETE on `feature/real-palletizer`** (18/18 green) — the MVP go/no-go **PASSED**. `demos/solve_workflow` is now a TWO-LINE robotic palletizing plant: one shared box feeder → a heavy cell (25 kg → KUKA KR30) + a light cell (5 kg → KR6), each with its own empty-pallet supply + a shared dispatch; six belts route clear (0 collisions), two distinct arms (no allocator collapse, no PARTIAL/orphan), `hard_constraints_satisfied`. Net-new **`validate_workflow()`** (`solver/workflow_validation`) is a report-only topology pass (duplicate-id / unknown-task / port-polarity / unfed-input Errors, undrained-output Warning) that the demo gates the solve on. Domain-credible (NOT certified) per-category clearances (~500 mm around robots) replace the 0.2 m placeholder, labelled in-code. (M1: annulus transport endpoints are polar `(r, θ)` placer variables, `r` hard-bounded to the reach band. M2: post-solve `route_belts()` builds a `PlacedBelt` per edge from `BeltSpec` catalog, belt-vs-station collision flagged.) **A `BeltStrategy` OR-tree competitor was deliberately NOT built** (`decisions.md` "Belt geometry …"). M3 merged to `dev` and pushed. **Step-6 M4 COMPLETE on `dev` — M4.1 + M4.2 + M4.3 (19/19 green).** M4.1: a palletizer station is a MULTI-EQUIPMENT cell — arm + real **pallet-zone footprint(s)**; the station footprint is the **accumulated convex hull** of its equipment, buffered, built caller-side via the boost adapter (`solver/` stays foreign-lib-free per §1). M4.2: the inter-station overlap term gains a **convex-polygon NARROW PHASE** (`overlap_penalty_poly`, pure-core SAT) so elongated cells pack to **footprint contact** instead of being over-spaced by bounding circles. M4.3 capstone: the intra-station sub-problem is REAL — a **dual-pallet cell** (one KR120 PA arm serving two pallet positions via task-sharing), laid out + feasibility-checked by net-new pure-solver `layout_palletizer_cell()` (`solver/station_template`, the crudest `StationTemplate`: templated slots on a reach arc + a fit/non-collision check, reports infeasible rather than fabricating). The demo refuses to draw an infeasible cell; the check also surfaces that only PA-class arms can cover a full EUR pallet. **Deferred (earned when a scenario needs it):** a **free intra-station NLP**, lazy **non-convex union** + `StationFootprint` cache (data-model.md §3.1), **hard `LN_COBYLA` overlap**, heterogeneous **multi-instance stations** (`decisions.md` M4.1/M4.2/M4.3). **MVP + the planned step-6 milestones are now complete; next is post-MVP (sync/ecs/render/gui, LNS) — none scaffolded, all parked.**
