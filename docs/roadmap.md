...
6. **Inner-problem realism — port regions, belt geometry, realistic palletizer scenario.** Originally scoped as LNS + simulated annealing; re-scoped 2026-05-27 after building LNS end-to-end and finding that with the existing inner problem (one-instance-per-station at origin, port positions at `(0,0)`, magical transports with no geometry) there were no genuine local optima to escape and SA never fired. LNS scaffold parked on `feature/step-6-lns` (decisions.md "MVP re-scoped"). The new step-6 milestones, each on its own feature branch:
   - **M1 — Port regions** (`feature/port-regions`). Goal: ports become placer variables; equipment-specific reachability and rigidity drive where they can land.
     ...
     *Concepts under discussion for M1 (not committed; subject to change before code lands):*
     ...
     - `PusherStrategy` emits its port as a Point at the stroke endpoint offset, perpendicular direction.
   ...
   - **M4 — Multi-instance stations + intra-station packing** (`feature/intra-station`). Builds the second placement problem the architecture has always anticipated (`solver.md` "Geometry flattening & caching"). When a palletizer cell has a pallet zone as separate equipment (not just a port at a fixed offset from the arm), the intra-station sub-problem becomes a real packing problem. The `StationFootprint` cache (data-model.md §3.1) is the natural data structure for it. *Concept under discussion: how much of the layout is templated (station-type-specific fixed offsets) vs free (small NLP over equipment within station). Real palletizer cells are largely templated in industry; if the templated approach is enough, the intra-station NLP shrinks to a feasibility check.* If M3 doesn't surface a need for M4 it stays a small, low-risk addition; if it does, M4 closes the gap.
