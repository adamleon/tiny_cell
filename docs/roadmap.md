...
6. **LNS + annealing** outer loop tying it together (`solver.md` outer loop). Next milestone.
...
## Step-5 deferrals (carry-overs from the Layer-3 build)

Listed in roughly priority order if step 6 (LNS) or interaction work surfaces a real need.

- **Bounding-circle collision uses footprint, not reach envelope.** Two arms can land with overlapping reach circles even though their footprints don't touch — wrong for real cells. The reach data is already in the catalog; the fix is one line in the LayoutProblem builder (use `max_radius` for arm collision instead of the footprint hull).
- **Transports don't drive the objective.** They're declared in workflows, recorded in AllocationResult, drawn on the SVG, but the placer doesn't yet have a transfer-length soft term. Requires threading `TransportEdge`s into `LayoutProblem` + a sum-of-distances objective term over the connected port world-poses.
- **Port direction tolerances unenforced by the placer.** Strategies emit `PortConstraint`s with direction + tolerance (pusher strict-perpendicular, arm loose); the placer ignores them. Requires either a per-Transport direction-alignment soft term or a hard constraint via `LN_COBYLA`.
- **Overlap residual feasibility.** Penalty-method equilibrium leaves sub-mm interpenetration when overlap and the prior compete (Phase E discovery; engineering-tolerant via the 1e-6 m² check). Real fix is converting overlap to an `LN_COBYLA` inequality constraint — one algorithm swap + one constraint per pair.
- **`StationFootprint` cache.** Data-model.md §3.1 specifies the hull + lazy-union + clearance-buffered station-footprint cache. Phase B deferred it because there was no consumer; LNS destroy-and-repair will need it for the narrow-phase collision check.
- **Intra-station equipment layout.** Current model is one-instance-per-station at the station origin. When a station gets multiple bound instances (e.g. arm + camera + fixture), the intra-station sub-problem from `solver.md` "two independent placement problems" lands then.
- **Workflow validation.** No `validate_workflow()` checks that every Palletize task has incoming Transports for its required ports. User chose to defer until GUI work — port-mismatches at MVP just produce silent missing edges.
