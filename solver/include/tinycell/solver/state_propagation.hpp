...
// or the first violation (failure). The state-flow rules themselves
// (Knowledge vs Control axes, alignment semantics) live in
// data-model.md §4.
//
...
// `propagate_state` is for the cases where something OTHER than a
// state-aware solver produced the candidate: late-stage validation of
...
// requires_state violation. `trajectory[i]` is the inbound state to
// task i (the state its requires_state was checked against).
// `trajectory.back()` is the final outbound state after the last task's
// effect. Size is always `workflow.size() + 1`.
struct PropagationSuccess {
    std::vector<core::ItemState> trajectory;
};

// PropagationFailure — one task's requires_state returned false.
// Identifies the failing task's index in the workflow, the strategy
// name whose predicate rejected, and the inbound state that failed.
// The caller can diff `inbound_state` against what the strategy needs
// to diagnose the gap. No partial trajectory is returned — the value
// of the failure surface is "this is where it broke," not "here's how
// far we got."
...
// strategy's requires_state against the running state; apply its effect
// to produce the state the next task observes. Returns the first
// violation as PropagationFailure, or the full trajectory as
// PropagationSuccess.
//
...
//                + effect — an empty std::function here is a defect in
//                the caller's selection logic (engineering.md §3), not
//                a user-visible failure mode, and the propagator throws
//                on it rather than fabricate identity behavior.
